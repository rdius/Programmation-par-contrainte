...
public class Chocobinpacking2D {
	...
	public boolean solveWithOrientation(int timeLimit) {
		...
		for (int i = 0; i < n; i++) {
			// constraintes de non depassement du conteneur
			// o[i] = 0
		...
		for (int i = 0; i < n - 1; i++) {
			for (int j = i + 1; j < n; j++) {
				...
				// o[i] = 0, o[j] = 0, lorsqu'il n'y a pas de rotation sur x et y
				...
				// o[i] = 0, o[j] = 1, rotation sur sur y
				m.addConstraint(Choco.implies(
				...
				// o[i] = 1, o[j] = 1, rotation sur x et y
				m.addConstraint(Choco.implies(
		...
		// verification de la solution trouvée
		if (ok) {
			s_x = new int[n];
			s_y = new int[n];
			s_o = new int[n];
	...
	public void outTableNew(String fn) {
		final String[] Color = new String[] { "#FFFF00", "#1CE6FF", "#FF34FF",
				"#FF4A46", "#008941", "#006FA6", "#A30059", "#FF0000",
				"#7A4900", "#0000A6", "#63FFAC", "#B79762", "#004D43",
				"#8FB0FF", "#997D87", "#5A0007", "#809693", "#1B4400",
				"#4FC601", "#3B5DFF", "#4A3B53", "#FF2F80", "#61615A",
				"#BA0900", "#6B7900", "#00C2A0", "#FFAA92", "#FF90C9",
				"#B903AA", "#D16100", "#FFDBE5", "#000035", "#7B4F4B",
	...
	public void testBatch(String filename, int nbTrials, int timeLimit) {
		Chocobinpacking2D bp = new Chocobinpacking2D();
	...
	public static void main(String[] args) {
		...
		bp.readData("data/bin-packing-2D-W10-H7-I6.txt");
		//bp.readData("data/bin-packing-2D-W10-H8-I6.txt");
		//bp.readData("data/bin-packing-2D-W19-H16-I21.txt");
		//bp.readData("data/bin-packing-2D-W19-H17-I21.txt");
		//bp.readData("data/bin-packing-2D-W19-H18-I21.txt");
		//bp.readData("data/bin-packing-2D-W19-H19-I21.txt");
		//bp.readData("data/bin-packing-2D-W19-H20-I21.txt");
		//bp.readData("data/bin-packing-2D-W19-H21-I21.txt");
		//bp.readData("data/bin-packing-2D-W19-H22-I21.txt");
		//bp.readData("data/bin-packing-2D-W19-H23-I21.txt");
		//bp.readData("data/bin-packing-2D-W19-H24-I21.txt");
		// si la résolution réussie, j'affiche le résultat
		if (bp.solveWithOrientation(500))
			bp.outTableNew("CHOCO-bin-packing-2D.html");
